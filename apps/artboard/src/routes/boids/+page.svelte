<script lang="ts">
  import { makeRectangle, type Segment } from "@artboard/graphics";
  ...
  const w: Segment = { from: new Vector(0, 0), to: new Vector(0, 0) };
  ...
  let segs = makeRectangle(new DOMRect(20, 20, 400, 300));
  console.log(segs);
